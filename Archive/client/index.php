<?php
	...
	if (count($url) == 2 && $url[1] != "create" && $url[0] == "t" && $router->getTitle() == "Create Thread") {
		...
		if (!(new ThreadsClass())->findThreadByUrl($url[1]))
			header("Location: /");
	}
	...
	if (count($url) == 2 && $router->getTitle() == "Account Edit" && !is_numeric($url[1]) && $url[1] != "edit") {
		...
		if (!(new UsersClass())->searchUserByID($url[1]))
			header("Location: /account");
	}
	...
	if (!empty($_GET['token']) && $router->getTitle() == "Register Confirm") {
		include $_SERVER["DOCUMENT_ROOT"].'/server/helperClasses/TokenHelperClass.class.php';

		$token = (new TokenHelperClass())->validateToken([$_GET['token'], 1]);
		if ($token["response"] == 400) header("Location: /");
	} else if (!empty($_GET['token']) && $router->getTitle() == "Restore Confirm") {
		require_once $_SERVER["DOCUMENT_ROOT"].'/server/helperClasses/TokenHelperClass.class.php';
		
		$token = (new TokenHelperClass())->validateToken([$_GET['token'], 0]);
		if ($token["response"] == 400) header("Location: /");
	}
...
<html lang="en">
<head>
	<?php require_once $_SERVER["DOCUMENT_ROOT"].'/server/headers/header.php'; ?>
	<title>JoLa |
		<?php 
			if (count($url) == 2 && $url[0] == "t" && $url[1] != "create") {
				require_once $_SERVER["DOCUMENT_ROOT"].'/server/controllers/ThreadsClass.class.php';
				echo (new ThreadsClass())->getTitle($url[1]);
			} else {
...
<body>
	...
	<header>
		<nav
        class="d-flex justify-content-md-between justify-content-center py-3" style="width: 75%; margin-left: auto; margin-right: auto">
		<a href="/" class="d-flex trademark align-items-center me-3">
			<i class="bi bi-globe-europe-africa"></i
				><span class="ms-2">JoLa</span>
			</a>
			<section class="text-end ms-3">
				<ul>
					<?php if (isset($_SESSION['IS_AUTHORIZED'])) { ?> 
						<li class="me-3">
							<a href="/notifications" class="header-figure"
							  ><i class="bi bi-megaphone"></i
							></a>
						  </li>
					<li>
						<a href="/account" class="d-inline-flex header-figure">
							<img class="image-header-account img-fluid" src="<?php echo $_SESSION['USER_IMAGE']; ?>" alt="<?php echo $_SESSION['USERNAME']; ?>-Profile-Picture" />
							<span class="ms-2"><?php echo $_SESSION['USERNAME']; ?></span>
						</a>
					</li>
					<li class="ms-3">
						<a href="/logout" class="header-figure">
					...
					<li>
						<a href="/login" class="effective-login-header header-figure">
							Sign in
						</a>
					</li>
					<?php } ?>
